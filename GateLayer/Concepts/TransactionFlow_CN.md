...
`op-batcher` 会从 Sequencer (定序器) 收集一批交易，并将它们聚合到一个 `channel` 中。通过将多个批次的数据聚合在 `channel` 里，可以获得更高的数据压缩率，从而降低单笔交易的成本。
...
*   **`safe` (安全)**：交易数据已经被打包并作为 `blob` 成功发布到 GateChain 的一个区块中。此时，交易数据已安全地记录在 L1 上。由于 GateChain 不会发生链重组，因此 `safe` 状态的交易数据不会丢失，但需要等待 L1 的最终确认才能达到 `finalized` 状态。
*   **`finalized` (最终确认)**：包含这笔交易数据的 GateChain 区块已经足够老，达到了 GateChain 的最终确认高度。一旦交易达到 `finalized` 状态，它就是不可逆的。
